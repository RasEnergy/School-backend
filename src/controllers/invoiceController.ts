...
export const invoiceController = {
  async getInvoices(req: Request, res: Response) {
    try {
      const { page = "1", limit = "10", status, branchId, paymentMethod, search } = req.query
    const user = req.user
      if (!user) {
  ...
  async exportInvoices(req: Request, res: Response) {
    try {
      ...
      const { status, branchId, paymentMethod, search, format = "excel" } = req.query
      ...
      const buffer = await invoiceService.exportInvoices(filters, user, format as string)
  ...
  async getInvoiceById(req: Request, res: Response) {
    ...
    try {
      ...
      const result = await invoiceService.confirmPayment(id, transactionReference, notes, user)
      ...
      if (error.message === "Invoice already paid" || error.message === "No pending payment found") {
  ...
  async resendPaymentLink(req: Request, res: Response) {
    try {
      ...
      if (error.message === "No pending online payment found" || error.message === "Parent phone number not found") {
        return res.status(400).json({ error: error.message })
      }
      res.status(500).json({ error: "Internal server error" })
    }
