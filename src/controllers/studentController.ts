...
export const getStudentsByBranch = async (req: Request, res: Response) => {
  try {
    ...
    const students = await studentService.getStudentsByBranch(branchId, gradeId as string)
...
export const searchStudents = async (req: Request, res: Response) => {
  try {
    ...
    if (!query || !branchId) {
      return res.status(400).json({ success: false, error: "Query and branchId are required" })
    }

    const students = await studentService.searchStudents(query as string, branchId as string)
    res.json({ success: true, data: students })
  } catch (error) {
...
export const registerStudent = async (req: Request, res: Response) => {
  try {
    ...
    if (
      ...
      return res.status(400).json({ success: false, error: "Missing pricing information" })
    }
    ...
    if (parentData.parentEmail && !validateEmail(parentData.parentEmail)) {
      return res.status(400).json({ success: false, error: "Invalid parent email format" })
    }
    ...
    if (!validateEthiopianPhone(parentData.parentPhone)) {
      return res.status(400).json({ success: false, error: "Invalid parent phone number format" })
    }

    // Validate date of birth
    if (studentData.dateOfBirth && !validateDateOfBirth(studentData.dateOfBirth)) {
    ...
    if (files?.studentPhoto?.[0]) {
      ...
      if (!validation.isValid) {
        return res.status(400).json({ success: false, error: `Student photo: ${validation.error}` })
      }
    ...
    if (files?.parentPhoto?.[0]) {
      ...
      if (!validation.isValid) {
        return res.status(400).json({ success: false, error: `Parent photo: ${validation.error}` })
      }
    ...
    if (user.role !== "SUPER_ADMIN" && user.branchId !== studentData.branchId) {
      return res.status(403).json({ success: false, error: "Access denied to this branch" })
    }
